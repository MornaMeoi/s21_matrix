...
START_TEST(null_ptr_check_s21_sub_matrix) {
  matrix_t rhs, result;
  s21_create_matrix(4, 3, &rhs);
  ck_assert_int_eq(s21_sub_matrix(NULL, &rhs, &result),
                   CODE_INCORRECT_MATRIX_ERROR);
  s21_remove_matrix(&rhs);
}
END_TEST

START_TEST(unequal_rows_check_s21_sub_matrix) {
  matrix_t lhs, rhs, result;
  s21_create_matrix(4, 4, &lhs);
  s21_create_matrix(3, 4, &rhs);
  ck_assert_int_eq(s21_sub_matrix(&lhs, &rhs, &result), CODE_CALCULATION_ERROR);
  s21_remove_matrix(&lhs);
  s21_remove_matrix(&rhs);
}
END_TEST

START_TEST(unequal_columns_check_s21_sub_matrix) {
  matrix_t lhs, rhs, result;
  s21_create_matrix(4, 4, &lhs);
  s21_create_matrix(4, 3, &rhs);
  ck_assert_int_eq(s21_sub_matrix(&lhs, &rhs, &result), CODE_CALCULATION_ERROR);
...
START_TEST(common_sub_matrix_check_s21_sub_matrix) {
  ...
  double lhs_test_data[] = {1.2, 3.4, -1.2, -3.4},
         ...
         result_test_data[] = {2.4, 6.8, -2.4, -6.8};
  ...
  ck_assert_int_eq(s21_sub_matrix(&lhs, &rhs, &result), CODE_OK);
...
Suite *s21_sub_matrix_suite() {
  ...
  tcase_add_test(tc_corner, null_ptr_check_s21_sub_matrix);
  tcase_add_test(tc_corner, unequal_rows_check_s21_sub_matrix);
  tcase_add_test(tc_corner, unequal_columns_check_s21_sub_matrix);
  tcase_add_test(tc_corner, common_sub_matrix_check_s21_sub_matrix);
